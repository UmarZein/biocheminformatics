...
class ConvLayer(MessagePassing):
    ...
    def __init__(
        ...
        dropout_rate=0.1, 
        final_dropout_rate=0.0,
        pretrain_edge_mlp=False,
        allow_train_edge_mlp=False,
    ):
        super().__init__(node_dim=0,aggr=aggr)
        if not (pretrain_edge_mlp or allow_train_edge_mlp):
            print("warning: edge_mlp will never be trained")
        self.pretrain_edge_mlp=pretrain_edge_mlp
        self.allow_train_edge_mlp=allow_train_edge_mlp
        self.node_mlp=(MLP(
            mlp_dims_node[0], 
            mlp_dims_node[1:-1], 
            mlp_dims_node[-1],
        ...
        self.edge_mlp = MLP(
            ...
            final_norm=nn.Identity,
            activation=activation, 
            final_activation=(lambda: nn.Softmax(-1)) if allow_train_edge_mlp else (nn.Sigmoid), 
            dropout_rate=dropout_rate, 
            final_dropout_rate=0,
        )
        
        self.gaussian_smear = GaussianSmearing(0.0, self.cutoff, mlp_dims_edge[-1])
        self.reset_parameters()
    ...
    def reset_parameters(self):
        ...
        if self.pretrain_edge_mlp:
            if self.get_cached_state_dict() is not None:
                self.edge_mlp.load_state_dict(self.get_cached_state_dict())
            else:
                self.edge_mlp.reset_parameters()
                opt=torch.optim.AdamW(self.edge_mlp.parameters())
                x=torch.linspace(0,self.cutoff,256)
                z=(x*2-self.cutoff).view(-1,1)/self.cutoff
                ...
                for _ in it:
                    ...
                    self.edge_mlp.zero_grad(set_to_none=True)
                    loss.backward()
                    opt.step()
                print('after:', loss.item())
                self.set_cached_state_dict(self.edge_mlp.state_dict())
        else:
            self.edge_mlp.reset_parameters()
        ...
        for i, (mlp_dims_node, mlp_dims_edge) in enumerate(zip(node_dimses, edge_dimses)):
            if i+1<len(node_dimses):
                self.convs.append(ConvLayer(
                    ...
                    pretrain_edge_mlp=pretrain_edge_mlp,
                    allow_train_edge_mlp=allow_train_edge_mlp,
                ))
            else:
                self.convs.append(ConvLayer(
                    mlp_dims_node, 
