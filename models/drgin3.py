...
class RGINConv(MessagePassing):
    ...
    def forward(self, x: Tensor,
        ...
        #edge_index: 2, |e|
        edge_attr = self.edge_mlp(edge_attr).tanh()#.softmax(-1)
...
class DRGIN3(nn.Module):
    ...
    def forward(self, 
        ...
        for i,(rgin, norm) in enumerate(zip(self.rgins, self.norms)):
            #print("h:",h.shape)
            h_ = rgin(h, edge_index, edge_attr, edge_weight=edge_weight)
            #print("h_:",h_.shape)
            if i+1<len(self.rgins):
                h_=norm(h,batch=data.batch)
                h_=h_.tanh()
                h=(h+h_)/2
            else:
                h=h_
