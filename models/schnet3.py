...
class Inner(torch.nn.Module):
    ...
    def __init__(
        ...
        self.rgins = ModuleList()
        for _ in range(num_rgins):
            rgin = RGINConv(
                [hidden_channels,hidden_channels//4,hidden_channels,],
                [num_gaussians,num_gaussians,4,]
            )
            self.rgins.append(rgin)
    ...
    def forward(self, z: Tensor, pos: Tensor,
        ...
        for rgin in self.rgins:
            h = h + rgin(h, bond_index, bond_emb)
