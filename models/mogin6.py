...
def load_balancing_loss(x,alpha,beta):
    #x: [#input, #experts]
    #alpha: rec. [0,1]
    #beta: rec. [0,1]
    expert_bias_loss=((x.mean(-2)**2).sum()-(1/x.shape[-1]))*(1/(1-1/x.shape[-1]))
    uncertainty_loss=1-(x**2).sum(-1).mean()
    return alpha*expert_bias_loss+beta*uncertainty_loss
#Mixture of GinConv
class MoGINConv(MessagePassing):
    ...
    def forward(self, x: Tensor,
        ...
        load_balacing_loss = load_balancing_loss(edge_attr, self.routing_loss_alpha, self.routing_loss_beta)
    ...
    def __init__(self, 
        ...
        for mlp_dims_node, mlp_dims_edge in zip(node_dimses, edge_dimses):
            self.convs.append(MoGINConv(
                ...
                dropout_rate=dropout_rate, aggr=aggr, 
                routing_loss_alpha=routing_loss_alpha, routing_loss_beta=routing_loss_beta))
